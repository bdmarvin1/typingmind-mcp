...
async function start(authToken) {
  ...
  app.get('/oauth/google/initiate', (req, res) => {
    ...
    const scopes = [
      ...
      'https://www.googleapis.com/auth/analytics.readonly', // <-- ADDED THIS LINE
  ...
  app.post('/restart/:id', auth, async (req, res) => {
    ...
    try {
      const config = clientEntry.config || { command: clientEntry.command, args: clientEntry.args, env: clientEntry.initialEnv }; // Use initialEnv
      if (clientEntry.client && typeof clientEntry.client.close === 'function') {
         await clientEntry.client.close(); 
      }
      clients.delete(id);
      const result = await startClient(id, config); // config here contains initialEnv
  ...
  // MODIFIED /clients/:id/call_tools route
  app.post('/clients/:id/call_tools', auth, validateUserContextMcpToken, async (req, res) => {
    ...
    if (!oauth2Client || !oauth2Client.credentials || !oauth2Client.credentials.refresh_token) {
      console.error(`call_tools: oauth2Client for Google user ${req.googleUserContext.email} (sub: ${req.googleUserContext.sub}) is not properly configured with a refresh token.`);
      return res.status(500).json({ 
        error: 'Server not configured to obtain Google Access Token. OAuth flow may be incomplete or refresh token missing for the authenticated user.',
        code: 'GOOGLE_AUTH_UNINITIALIZED'
      });
    }
    
    console.log(`call_tools: Authorized by main token. User context token validated for sub=${req.googleUserContext.sub}, email=${req.googleUserContext.email}`);
    ...
    try {
      ...
      console.log(`call_tools: Calling tool '${toolName}' on client '${downstreamClientId}' with token injected into arguments.`);
      const result = await clientEntry.client.callTool({
        name: toolName,
        arguments: augmentedToolArgs,
      });

      res.status(200).json(result);

    } catch (error) {
      console.error(`call_tools: Error during Google Access Token retrieval or calling tool for client ${downstreamClientId}:`, error);
      ...
      return res.status(500).json({
        error: 'Failed to call tool due to an internal error related to Google authentication or tool execution.',
        code: 'CALL_TOOL_INTERNAL_ERROR',

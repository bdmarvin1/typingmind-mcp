...
const GOOGLE_CLOUD_PROJECT = process.env.GOOGLE_CLOUD_PROJECT; // For constructing secret parent path

// Secret Manager Configuration for Google Refresh Token
// For loading: projects/PROJECT_ID/secrets/SECRET_ID/versions/latest
// For storing (parent): projects/PROJECT_ID/secrets/SECRET_ID
...
async function start(authToken) {
  ...
  if (GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET && GOOGLE_REDIRECT_URI) {
    ...
    // Load refresh token on startup if configured and oauth2Client is available
      ...
      if (tokens.refresh_token) {
        ...
        if (GOOGLE_REFRESH_TOKEN_SECRET_PARENT_FOR_STORE && GOOGLE_CLOUD_PROJECT) {
          try {
            ...
            oauth2Client.setCredentials({ refresh_token: tokens.refresh_token }); // Also set on current client
            refreshMessage += 'Successfully stored in Secret Manager and set on OAuth client.';
            console.log(refreshMessage); // Log success specifically
          } catch (secretError) {
            console.error('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
            console.error('Failed to store new refresh token in Secret Manager:');
            console.error('Error Code:', secretError.code); // gRPC error code
            console.error('Error Details:', secretError.details); // More details if available
            console.error('Full Secret Error Object:', JSON.stringify(secretError, null, 2)); // Stringify the whole error
            console.error('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
        ...
        refreshMessage += 'No new refresh token. OAuth client might already have one or an error occurred.';
      }
  ...
  app.post('/restart/:id', auth, async (req, res) => {
    ...
    try {
      const config = clientEntry.config || { command: clientEntry.command, args: clientEntry.args, env: clientEntry.env };

...
const GOOGLE_AWARE_CLIENT_IDS = ['mcp-server-gsc']; // Add other GSC or Google-dependent client IDs here
...
async function start(authToken) {
  ...
  app.post('/clients/:id/call_tools', auth, validateUserContextMcpToken, async (req, res) => {
    ...
    // Only attempt to get Google Access Token if the client is Google-aware
    ...
    // This try-catch is for the clientEntry.client.callTool() itself
    try {
        ...
        // Assuming toolError might be an error object from the MCP SDK or the downstream client process
        return res.status(500).json({
